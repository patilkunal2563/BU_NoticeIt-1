module Notices
  class OrdersController < Notices::ApplicationController
  
    before_filter { @active_nav = :orders }  
    before_filter { params[:id] && @order = Notices::Order.find(params[:id])}
  
    def index
      @query = Notices::Order.ordered.received.includes(:order_items => :ordered_item).page(params[:page]).search(params[:q])
      @orders = @query.result
    end
    
    def new
      @order = Notices::Order.new
      @order.order_items.build(:ordered_item_type => 'Notices::Product')
    end
    
    def create
      Notices::Order.transaction do
        @order = Notices::Order.new(safe_params)
        @order.status = 'confirming'
        if !request.xhr? && @order.save
          @order.confirm!
          redirect_to @order, :notice => "Notice has been created successfully"
        else
          @order.order_items.build(:ordered_item_type => 'Notices::Product')
          render :action => "new"
        end
      end
    rescue Notices::Errors::InsufficientStockToFulfil => e
    ...
    rescue Notices::Errors::PaymentDeclined => e
    ...
    def despatch_note
      render :layout => 'notices/printable'
    end
