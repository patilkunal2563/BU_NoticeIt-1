module Notices
  class ProductCategoriesController < Notices::ApplicationController

   
    before_filter { @active_nav = :product_categories }  
    before_filter { params[:id] && @product_category = Notices::ProductCategory.find(params[:id]) }
  
    def index
      @product_categories = Notices::ProductCategory.ordered.all
    end
  
    def new
      @product_category = Notices::ProductCategory.new
    end
  
    def create
      @product_category = Notices::ProductCategory.new(safe_params)
